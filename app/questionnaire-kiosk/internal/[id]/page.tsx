...
export default async function InternalDocumentationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const session = await prisma.patientQuestionnaireSession.findUnique({ where: { id }, select: { status: true, session_kind: true, internal_workflow_id: true, owner_practice_id: true, created_by_kiosk_device_id: true, frozen_blocks: true, patient_reference: true } });
