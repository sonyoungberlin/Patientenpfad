...
export function KioskCheckInActions({ sessionId }: { sessionId: string }) {
  ...
  async function submit(action: "close" | "start_questionnaire") {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/questionnaire/${sessionId}/kiosk-handoff`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          action,
          ...(action === "start_questionnaire" ? { selected_block_ids: selectedBlockIds } : {}),
        }),
      });
  ...
  return (
    <section data-kiosk-check-in-actions={sessionId} style={{ display: "grid", gap: "0.5rem" }}>
      <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
        <button type="button" disabled={pending} onClick={() => void submit("close")}>
          Check-in abschließen
        </button>
        <button type="button" disabled={pending} onClick={() => setSelecting(true)}>
      ...
      {selecting ? (
        <div style={{ display: "grid", gap: "0.5rem" }}>
          ...
          <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
            ...
            >
              {pending ? "Wird bereitgestellt…" : "Am Kiosk bereitstellen"}
            </button>
          </div>
        </div>
      ) : null}
