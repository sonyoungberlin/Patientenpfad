...
type Device = { id: string; name: string; is_active: boolean; revoked_at: string | Date | null; created_at: string | Date; last_seen_at: string | Date | null };
...
export default function QuestionnaireKioskDeviceSettings({ initialDevices }: { initialDevices: Device[] }) {
  ...
  async function activate(event: React.FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/practice/questionnaire-kiosk-devices", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(Object.fromEntries(form)) });
    const data = await response.json() as { error?: string };
    if (response.ok) window.location.replace("/questionnaire-kiosk/lock"); else setError(data.error ?? "Einrichtung fehlgeschlagen.");
  }
  async function action(id: string, actionName: string, pin?: string) {
    ...
    if (!response.ok) { const data = await response.json() as { error?: string }; setError(data.error ?? "Aktion fehlgeschlagen."); return; }
